            ...
            <form onSubmit={submit}>
              ...
              {tab==='payment' && <>
                <div className="alert alert-info" style={{marginBottom:12, fontSize:12}}>
                  <b>Advance %, Supply %, Installation %, Commissioning %</b> together define how a
                  BOQ item's total value splits across stages (should sum to 100%). Used by
                  <b> BOQ Manager → Add Split Item</b> to auto-calculate each stage's rate from one
                  total value, and by RA Bill's per-bill advance recovery rate.
                </div>
                <div className="form-grid form-grid-3">
                ...
                {(() => {
                  const sum = (parseFloat(form.pt_advance_pct)||0) + (parseFloat(form.pt_lc_pct)||0) + (parseFloat(form.pt_installation_pct)||0) + (parseFloat(form.pt_commissioning_pct)||0)
                  const ok = Math.abs(sum - 100) < 0.01
                  return (
                    <div className={`alert ${ok ? 'alert-success' : 'alert-warning'}`} style={{marginBottom:12, fontSize:12}}>
                      Advance + Supply + Installation + Commissioning = <b>{sum.toFixed(1)}%</b>
                      {ok ? ' ✓ Adds up to 100%' : ' — should total 100% for correct item value split'}
                    </div>
                  )
                })()}
                <div className="alert alert-info" style={{marginBottom:12, fontSize:12}}>
                  <b>Note:</b> the actual advance amount received from the client is no longer entered
                  here — it's rarely known at project setup. Once Accounts confirms the advance has
                  been received, record it (one time) from the <b>RA Bill page → Record Advance
                  Received</b>. That entry drives automatic advance recovery on future RA bills.
                </div>
