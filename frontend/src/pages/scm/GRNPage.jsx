...
export default function GRNPage() {
  ...
  const submit = async e => {
    ...
    setSubmitting(true)
    try {
      const res = await grn.create(activeProject.id, {
        ...form,
        qty_received: parseFloat(form.qty_received)
      })
      toast.success(`${res.data.grn_number} saved — email sent to Accounts & Site`)
      setForm({
        grn_date: today(), boq_item_id:'', qty_received:'',
        vendor_name:'', challan_no:'', vehicle_no:'', remarks:''
      })
      ...
      toast.error(e.response?.data?.error || 'Error creating GRN — please try again')
    } finally {
      setSubmitting(false)
    }
  }
  ...
  const deleteGRN = async (id, grn_number) => {
    ...
    try {
      ...
      toast.error('Could not delete — Admin access required')
    }
  }
      ...
      <div className="card">
        ...
        <div className="table-wrap">
          <table>
            ...
            <tbody>
              {loading && (
                <tr><td colSpan={isAdmin?9:8} className="empty">Loading...</td></tr>
              )}
